...
dfTrain <- data.frame(
  income = c(34, 67, 70, 110, 89, 90, 102, 104, 110, 120, 170),
...
# Plot data
dfTrain %>% ggplot(aes(x = CreditRating, y = income, color = Qualify)) +
  geom_point()

# Create imaginary credit application
...
df <- data.frame(Sepal.Length = 6.20, 
                 Sepal.Width = 2.80)
...
dfTrain <- data.frame(
  income = c(34000, 67000, 70000, 110000, 89000, 90000, 102000, 104000, 110000, 120000, 170000),
  CreditRating = c(750, 680, 670, 675, 710, 690, 715, 720, 650, 710, 720),
  Qualify = c("Yes", "No", "No", "Yes", "No", "No", "Yes", "Yes", "No", "Yes", "Yes")
)

classifications <- knn.cv(dfTrain[, 1:2], dfTrain$Qualify, k = 3)
confusionMatrix(classifications, as_factor(dfTrain$Qualify))
...
credit <- read_csv(here::here("6306", "week 06", "Credit Default.csv"))

# make resposnse a factor rather than 0,1
credit$`default payment next month` <-
  factor(credit$`default payment next month`,
         labels = c("NoDefault", "Default"))
credit$`default payment next month` <- as_factor(credit$`default payment next month`)

# plot the data
credit %>% 
  ggplot(aes(x = AGE, y = LIMIT_BAL, color = `default payment next month`)) +
  geom_point()
...
```{r}
pottery <-
  readxl::read_excel(here::here("6306", "week 06", "potterydata.xlsx"))
pottery
