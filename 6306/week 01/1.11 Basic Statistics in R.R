...
# Another way to build data frames with cbind
df4 <- cbind(df1,df2)
df4
...
Example1 <- read.csv(here::here("6306", "week 01", "BusinessSales.csv"),header = TRUE)
Example1$sales
...
# We want a barplot of frequency of each class of car. Note here how the data is
# in a different format for mpg. It is not one row per relevant observation with
# the value at the end. We need to actually count the number of each class and
# then plot. We could count ... that would take a while. We could turn class
# into a factor and then let summary count for us.
...
# Draw a sample from a standard normal distribution run many times varying
# sample size and look at histogram and mean.
...
# to take and thus the number of sample means to return as well as the mean and
# sd. Define these outside the function so that they exist outside the function
# (global).
...
# Now adapt the code above to generate a specified number of sample means of a
# specified size sample size (this has already been done) from a normally
# distributed population with a specified mean and standard deviation (currently
# it is forced to be from a standard normal distribution N(0,1)).

# Run it a few times and use it generate 10000 sample means, each from a sample
# of size 50, from a normal distribution with mean = 60 and sd = 10.  Provide a
# distribution (histogram) of the distributions as well.

# What is the shape of the distribution?  What is the mean and sd?  What should
# they be given the Central Limit Theorem (CLT).  For a refresher of the CLT see
# the Stats Bridge Course!
